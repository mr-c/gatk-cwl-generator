...
doc: "\n\n <p>\n Given a VCF with genotype likelihoods from the HaplotypeCaller, UnifiedGenotyper,\
  \ or another source which provides\n <b>unbiased</b> genotype likelihoods, calculate\
  \ the posterior genotype state and likelihood given allele frequency\n information\
  \ from both the samples themselves and input VCFs describing allele frequencies\
  \ in related populations.</p>\n\n <p>The AF field will not be used in this calculation\
  \ as it does not provide a way to estimate the confidence interval\n or uncertainty\
  \ around the allele frequency, while AN provides this necessary information. This\
  \ uncertainty is\n modeled by a Dirichlet distribution: that is, the frequency is\
  \ known up to a Dirichlet distribution with\n parameters AC1+q,AC2+q,...,(AN-AC1-AC2-...)+q,\
  \ where \"q\" is the global frequency prior (typically q << 1). The\n genotype priors\
  \ applied then follow a Dirichlet-Multinomial distribution, where 2 alleles per\
  \ sample are drawn\n independently. This assumption of independent draws is the\
  \ assumption Hardy-Weinberg Equilibrium. Thus, HWE is\n imposed on the likelihoods\
  \ as a result of CalculateGenotypePosteriors.</p>\n\n <h3>Input</h3>\n <p>\n   \
  \  <ul>\n         <li>A VCF with genotype likelihoods, and optionally genotypes,\
  \ AC/AN fields, or MLEAC/AN fields</li>\n         <li>(Optional) A PED pedigree\
  \ file containing the description of the individuals relationships.</li>\n     </ul>\n\
  \ </p>\n\n <p>\n A collection of VCFs to use for informing allele frequency priors.\
  \ Each VCF must have one of\n </p>\n <ul>\n     <li>AC field and AN field</li>\n\
  \     <li>MLEAC field and AN field</li>\n     <li>genotypes</li>\n </ul>\n </p>\n\
  \n <h3>Output</h3>\n <p>A new VCF with:</p>\n <ul>\n     <li>Genotype posteriors\
  \ added to the genotype fields (\"PP\")</li>\n     <li>Genotypes and GQ assigned\
  \ according to these posteriors</li>\n     <li>Per-site genotype priors added to\
  \ the INFO field (\"PG\")</li>\n     <li>(Optional) Per-site, per-trio joint likelihoods\
  \ (JL) and joint posteriors (JL) given as Phred-scaled probability\n  of all genotypes\
  \ in the trio being correct based on the PLs for JL and the PPs for JP. These annotations\
  \ are added to\n  the genotype fields.</li>\n </ul>\n\n <h3>Notes</h3>\n <p>\n Using\
  \ the default behavior, priors will only be applied for each variants (provided\
  \ each variant has at least 10\n called samples.) SNP sites in the input callset\
  \ that have a SNP at the matching site in the supporting VCF will have\n priors\
  \ applied based on the AC from the supporting samples and the input callset (unless\
  \ the --ignoreInputSamples\n flag is used). If the site is not called in the supporting\
  \ VCF, priors will be applied using the discovered AC from\n the input samples (unless\
  \ the --discoveredACpriorsOff flag is used). Flat priors are applied for any non-SNP\
  \ sites in\n the input callset.\n </p>\n\n <h3>Usage examples</h3>\n <h4>Inform\
  \ the genotype assignment of NA12878 using the 1000G Euro panel</h4>\n <pre>\n java\
  \ -jar GenomeAnalysisTK.jar \\\n   -T CalculateGenotypePosteriors \\\n   -R reference.fasta\
  \ \\\n   -V NA12878.wgs.HC.vcf \\\n   -supporting 1000G_EUR.genotypes.combined.vcf\
  \ \\\n   -o NA12878.wgs.HC.posteriors.vcf \n </pre>\n\n <h4>Refine the genotypes\
  \ of a large panel based on the discovered allele frequency</h4>\n <pre>\n java\
  \ -jar GenomeAnalysisTK.jar \\\n   -T CalculateGenotypePosteriors \\\n   -R reference.fasta\
  \ \\\n   -V input.vcf \\\n   -o output.withPosteriors.vcf\n </pre>\n\n <h4>Apply\
  \ frequency and HWE-based priors to the genotypes of a family without including\
  \ the family allele counts\n in the allele frequency estimates the genotypes of\
  \ a large panel based on the discovered allele frequency</h4>\n <pre>\n java -jar\
  \ GenomeAnalysisTK.jar \\\n   -T CalculateGenotypePosteriors \\\n   -R reference.fasta\
  \ \\\n   -V input.vcf \\\n   -o output.withPosteriors.vcf \\\n   --ignoreInputSamples\n\
  \ </pre>\n\n <h4>Calculate the posterior genotypes of a callset, and impose that\
  \ a variant *not seen* in the external panel\n is tantamount to being AC=0, AN=100\
  \ within that panel</h4>\n <pre>\n java -jar GenomeAnalysisTK.jar \\\n   -T CalculateGenotypePosteriors\
...
- doc: The de novo mutation prior
  ...
  inputBinding:
    prefix: --deNovoPrior
- doc: 'Do not use discovered allele count in the input callset for variants that
    do not appear in the external callset. '
  id: discoveredACpriorsOff
  type: boolean?
  inputBinding:
    prefix: --discoveredACpriorsOff
- doc: 'Do not use discovered allele count in the input callset for variants that
    do not appear in the external callset. '
  id: discoveredACpriorsOff
  type: boolean?
...
- doc: The global Dirichlet prior parameters for the allele frequency
  ...
  inputBinding:
    prefix: --globalPrior
- doc: Use external information only; do not inform genotype priors by the discovered
    allele frequency in the callset whose posteriors are being calculated. Useful
    for callsets containing related individuals.
  id: ignoreInputSamples
  type: boolean?
  inputBinding:
    prefix: --ignoreInputSamples
- doc: Use external information only; do not inform genotype priors by the discovered
    allele frequency in the callset whose posteriors are being calculated. Useful
    for callsets containing related individuals.
  id: ignoreInputSamples
  ...
  inputBinding:
    prefix: --ignoreInputSamples
- doc: The number of homozygous reference to infer were seen at a position where an
    "other callset" contains no site or genotype information
  id: numRefSamplesIfNoCall
  type: int?
  inputBinding:
    prefix: --numRefSamplesIfNoCall
- doc: The number of homozygous reference to infer were seen at a position where an
    "other callset" contains no site or genotype information
  id: numRefSamplesIfNoCall
  type: int?
...
- doc: Other callsets to use in generating genotype posteriors
  ...
  - File
  inputBinding:
    valueFrom: $(applyTagsToArgument("--supporting", inputs['supporting_tags']))
- type:
  - 'null'
  - type: array
    items:
    ...
    - type: array
      items: string
  doc: A argument to set the tags of 'supporting'
  id: supporting_tags
- doc: Other callsets to use in generating genotype posteriors
  id: supporting
  type:
