...
- doc: Allele Frequency selection mode
  ...
  inputBinding:
    prefix: --frequencySelectionMode
- doc: If true, will ignore genotypes in VCF, will take AC,AF from annotations and
    will make no sample selection
  id: ignoreGenotypes
  type: boolean?
  inputBinding:
    prefix: --ignoreGenotypes
- doc: If true, will ignore genotypes in VCF, will take AC,AF from annotations and
    will make no sample selection
  id: ignoreGenotypes
  type: boolean?
  inputBinding:
    prefix: --ignoreGenotypes
- doc: If true, will ignore polymorphic status in VCF, and will take VCF record directly
    without pre-selection
  id: ignorePolymorphicStatus
  type: boolean?
  inputBinding:
    prefix: --ignorePolymorphicStatus
- doc: If true, will ignore polymorphic status in VCF, and will take VCF record directly
    without pre-selection
  id: ignorePolymorphicStatus
  type: boolean?
...
- doc: File to which variants should be written
  ...
  inputBinding:
    prefix: --out
- doc: Regular expression to select many samples from the ROD tracks provided. Can
...
- doc: Regular expression to select many samples from the ROD tracks provided. Can
  ...
  inputBinding:
    valueFrom: $(generateArrayCmd("--sample_expressions"))
- doc: File containing a list of samples (one per line) to include. Can be specified
    multiple times
  id: sample_file
  type:
  - 'null'
...
- doc: Sample selection mode
  ...
  inputBinding:
    prefix: --sampleMode
- doc: 'GL-based selection mode only: the probability that a site is non-reference
    in the samples for which to include the site'
  id: samplePNonref
  type: double?
  inputBinding:
    prefix: --samplePNonref
- doc: 'GL-based selection mode only: the probability that a site is non-reference
    in the samples for which to include the site'
  id: samplePNonref
  type: double?
  inputBinding:
    prefix: --samplePNonref
- doc: Select only a certain type of variants from the input file. Valid types are
    INDEL, SNP, MIXED, MNP, SYMBOLIC, NO_VARIATION. Can be specified multiple times
